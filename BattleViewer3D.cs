...
namespace Yukar.Battle
{
    ...
    public class BattleViewer3D : BattleViewer
    {
        ...
        {
            public BattleUI(GameMain owner, Catalog catalog, BattleSequenceManager battleSequenceManager)
            {
                this.owner = owner;
                ...
                defaultItemList = LoadLayout(Rom.LayoutProperties.LayoutNode.UsageInGame.BattleItem);
                defaultSkillList = LoadLayout(Rom.LayoutProperties.LayoutNode.UsageInGame.BattleSkill);
                ...
                particleView = new SharpKmyGfx.GameView();
                particleView.setDisplayID(Util.BATTLEPARTICLEDISPLAYID);

                var sw = catalog.getGameSettings().screenWidth;
                var sh = catalog.getGameSettings().screenHeight;
                particleView.addCaptureFrame(sw, sh);//キャプチャするサイズ
                particleView.setViewportSize(sw, sh);//ビューポートサイズ
                particleView.setShadowMapScale(0.1f);//シャドウマップスケールを落としておく
                particleView.enableDynamicResolution(false);//動的解像度を使用しない
                particleView.setCamera(SharpKmyMath.Matrix4.ortho(0, sw * PARTICLE_SCALE, 0, -sh * PARTICLE_SCALE, -100, 100), SharpKmyMath.Matrix4.inverse(SharpKmyMath.Matrix4.translate(0, 0, 50)));
                // DOFと背景色は抜きの邪魔になるので戻す
                // The DOF and background color get in the way of extraction, so I change them back.
                particleView.setBackGroundColor(new SharpKmyGfx.Color(0, 0, 0, 0));
            ...
            internal LayoutDrawerController defaultItemList;
            internal LayoutDrawerController defaultSkillList;
            internal LayoutDrawerController result;
            private SharpKmyGfx.GameView particleView;
            internal SharpKmyGfx.Texture particleViewTexture;
            internal const float PARTICLE_SCALE = 0.005f;

            LayoutDrawerController currentItemList;
            LayoutDrawerController currentSkillList;

            internal LayoutDrawerController itemList
            {
                get
                {
                    return (currentItemList == null) ? defaultItemList : currentItemList;
                }
                set
                {
            ...
            internal LayoutDrawerController skillList
            {
                get
                {
                    return (currentSkillList == null) ? defaultSkillList : currentSkillList;
                }
                set
                {
                    currentSkillList = value;
                }
            }
            ...
            internal void setBattleSequenceManager(BattleSequenceManager battleSequenceManager)
            {
                main.setBattleSequenceManager(battleSequenceManager);
                command.setBattleSequenceManager(battleSequenceManager);
                message.setBattleSequenceManager(battleSequenceManager);
                defaultItemList.setBattleSequenceManager(battleSequenceManager);
                defaultSkillList.setBattleSequenceManager(battleSequenceManager);
                result.setBattleSequenceManager(battleSequenceManager);
            }
            ...
            internal class LayoutDrawerController
            {
                ...
                internal void setBattleSequenceManager(BattleSequenceManager battleSequenceManager)
                {
                    this.battleSequenceManager = battleSequenceManager;
                }
                ...
                public List<ItemFilter.FilterTagProperty> FilterTagProperties;
                ...
                internal void LoadLayout(Rom.LayoutProperties layouts, GameMain owner, Catalog catalog, Rom.LayoutProperties.LayoutNode.UsageInGame usageInGame)
                {
                    ...
                    if (layout != null)
                    {
                        LoadLayout(owner, catalog, layout, usageInGame);
                    }
                }

                internal void LoadLayout(GameMain owner, Catalog catalog, Rom.LayoutProperties.LayoutNode layout, Rom.LayoutProperties.LayoutNode.UsageInGame usageInGame)
                {
                    ...
                    {
                        autoSort = layout.AutoSort;
                    }
                    sortType = layout.SortType;
                    FilterTagProperties = ItemFilter.CreateFilteringTagFromText(layout.FilteringTag);
                }

                internal void AutoSort()
                {
                    if (!autoSort) return;
                    if (usageInGame == Rom.LayoutProperties.LayoutNode.UsageInGame.BattleItem)
                    {
                        gameMain.data.party.SortItem(sortType);
                    }
                    if (usageInGame == Rom.LayoutProperties.LayoutNode.UsageInGame.BattleSkill)
                    {
                        // パーティー全体のスキルのソートに変更
                        // Changed to skill sorting for the whole party
                        foreach (var player in gameMain.data.party.PlayersInMenu)
                        {
                            player.SortSkill(gameMain.catalog, sortType);
                        }
                        if (User != null)
                        {
                            battleSequenceManager.MakeSkillList();
                            SetSelectableState(battleSequenceManager.battleViewer.skillSelectWindowDrawer.GetChoicesData().Select(x => x.enable));
                        }
                    }
                }
                ...
                public List<Rom.NSkill> FilteredSkills { get { return gameContent.FilteredSkills; } set { gameContent.FilteredSkills = value; } }
                
                public Common.GameData.Hero FilteredSkillUser { get { return gameContent.FilteredSkillUser; } set { gameContent.FilteredSkillUser = value; } }
                ...
                internal bool IsHideContainerWhenOpenChild()
                {
                    return drawer.IsHideMyself();
                }
                ...
                internal AbstractRenderObject GetAnchorNode(string v)
                {
                    return drawer.GetRenderObjectWithPositionAnchorTag(v);
                }

                internal bool HasContent()
                {
                    var res = drawer.GetRenderObjects().Exists(x => x.MenuItem.visible);
                    return res;
                }
            }
            ...
            internal void finalize()
            {
                GameMain.instance.gameView.removeSubGameView(particleView);
                particleView.Dispose();
                particleView = null;
                main.finalize();
                command.finalize();
                message.finalize();
                defaultItemList.finalize();
        ...
        internal BattleViewer3D(GameMain owner) : base(owner)
        {
            ...
            ui = new BattleUI(owner, owner.catalog, this.owner);
        ...
        {
            this.owner = owner;
            ui.setBattleSequenceManager(owner);
        }
        ...
        internal override void BattleStart(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            {
                PlayCameraAnimation(Rom.Camera.NAME_BATTLE_START);
            ...
            ui.defaultItemList.ResetGameContent();
            ui.defaultSkillList.ResetGameContent();
        ...
        internal override void Draw(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            if (visibleItemList)
            {
                ...
                if (ui.itemList.Index >= 0 &&  owner.commandSelectPlayer.haveItemList.Count > ui.itemList.Index)
                {
            ...
            if (visibleSkillList)
            {
                ...
                if (ui.skillList.Index >= 0 && owner.commandSelectPlayer.useableSkillList.Count > ui.skillList.Index)
                {
            ...
            // スキルやアイテム選択中でもコマンドリストを表示する場合に対応
            // Supports displaying the command list even while selecting a skill or item.
            if (!visibleCommand && !ui.command.IsHideContainerWhenOpenChild() && displayWindow != WindowType.None && displayWindow != WindowType.MessageWindow)
            {
                visibleCommand = true;
                lockCommand = true;
            }
            ...
            if (visibleCommand)
            {
                ...
                if (index >= 0 && choicesData.Count > index)
                {
        ...
        internal override bool HasNoMessageWindow()
        {
            return !ui.message.HasContent();
        }
            ...
            if (!playerEffectDrawer.isEndPlaying)
            {
                ...
                else
                {
                    ...
                    foreach (var target in effectDrawTargetPlayerList)
                    {
                        ...
                        var index = owner.PlayerViewDataList.IndexOf(target as BattlePlayerData);
                        ...
                        {
                            const int HEIGHT_OFFSET = 80;
                            var pos = targetLayoutNode.GetOriginPosition();
                            playerEffectDrawer.draw((int)pos.X, (int)pos.Y, false);
                            // 3D座標から求める場合の例
                            // Example of finding from 3D coordinates
                            //var near = Util.UnProject2World(p, v, new SharpKmyMath.Vector3(pos.X, pos.Y + HEIGHT_OFFSET, -1), Graphics.ScreenWidth, Graphics.ScreenHeight);
                            //var far = Util.UnProject2World(p, v, new SharpKmyMath.Vector3(pos.X, pos.Y + HEIGHT_OFFSET, 1), Graphics.ScreenWidth, Graphics.ScreenHeight);
                            //var direction = SharpKmyMath.Vector3.normalize(far - near);
                            //var unprojected = near + direction * 10;// 計算結果 / Calculation result
                            var actor = searchFromActors(target);
                            if (actor != null)
                            {
                                var pos3dBack = actor.mapChr.pos;
                                actor.mapChr.pos = new Vector3(pos.X * BattleUI.PARTICLE_SCALE, -(pos.Y + HEIGHT_OFFSET) * BattleUI.PARTICLE_SCALE, 0);
                                if (playerEffectDrawer is EffectDrawer3D e3d)
                                    e3d.setDisplayId(Util.BATTLEPARTICLEDISPLAYID);
            ...
            if (!defeatEffectDrawer.isEndPlaying)
            {
                ...
                else
                {
                    ...
                    foreach (var target in defeatEffectDrawTargetList)
                    {
                        ...
                        var actor = searchFromActors(target);
                        defeatEffectDrawer.drawFor3D(skillUser?.mapChr, actor?.mapChr, defeatEffectDrawTargetList.Count, rot.Y);
        ...
        private void drawEnemyInfo(List<BattleEnemyData> enemyMonsterData)
        {
            ...
            // EnemyDataからは消えている可能性がある
            // It may have disappeared from EnemyData
            foreach (BattleEnemyData monster in fadeoutEnemyList.Where(x => !enemyMonsterData.Contains(x)))
            {
                var actor = searchFromActors(monster);
                if (actor == null)
                    continue;

                // ViewDataから消えているアクターは「透明にしない」オンであっても消す(バトルイベントで消去した場合に該当)
                // Actors that disappear from ViewData will be deleted even if \
                //if (!monster.monster.visibleWhenKO)
                actor.setOpacityMultiplier(Math.Min(1, monster.imageAlpha));
            }
        ...
        internal BattleActor AddPartyMember(BattlePlayerData data, Vector3[] playerLayouts)
        {
            ...
            if (playerLayouts != null && playerLayouts.Length > empty)
            {
            ...
            if (damageTextList != null && damageTextList.Count() > 0)
            {
                    ...
                    .OrderByDescending(info => info.targetCharacter.EffectPriority))
                {
                            ...
                            if (tgt is MenuSubContainer)
                            {
								switch (info.type)
								{
									...
									case BattleDamageTextInfo.TextType.Damage:
                                        shake = info.statusId == catalog.getGameSettings().maxHPStatusID;
                                        break;
									default:
										break;
								}
                    ...
                    foreach (var characterInfo in info.textInfo)
                    {
                        ...
                        if(info.id == -1)
                        {
                            int count = damageTextList.Count();
                            for (int i = 0; i < count; i++)
                            {
                                if (damageTextList.FirstOrDefault(x => x?.targetCharacter == info.targetCharacter && x?.id == i) == null)
                                {
                                    info.id = i;
                                    break;
                                }
                        ...
                        position.Y += Graphics.GetDivHeight(damageNumberImageId) * info.id;
        ...
        void UpdateBattleActors(List<BattleActor> inBattleActors, float inAngleY, bool inIsFriend)
        {
            foreach (var actor in inBattleActors)
            {
                ...
                if ((state != prevState || option != prevOption) &&
                    // FATAL_DAMAGE か KO のときに出すが、直前のStateがFATAL_DAMAGEだった場合はすでに表示済みのため、KOでは出さない
                    // It will be issued when FATAL_DAMAGE or KO occurs, but if the previous state was FATAL_DAMAGE, it will not be issued when KO occurs because it has already been displayed.
                    ((state == BattleActor.ActorStateType.KO && prevState != BattleActor.ActorStateType.FATAL_DAMAGE) || state == BattleActor.ActorStateType.FATAL_DAMAGE) &&
                    // 逃げたあとに再表示されることがある
                    // It may reappear after escaping.
                    prevState != BattleActor.ActorStateType.ESCAPE &&
        ...
        internal override void Update(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                var turnMark = ui.main.GetAnchorNode(string.Format("CtbSelected[{0}]", i));
        ...
        private void executeAction(BattleCharacterBase self, bool action, bool start)
        {
            ...
            if (action)
            {
                ...
                // actor が居なくなっていてもやらなければならない操作を先に行う
                // Perform operations that must be performed first even if the actor is not present
                switch (self.selectedBattleCommandType)
                {
                    ...
                    case BattleCommandType.Skip:
                        if (!start)
                        {
                            ...
                            // Actorが既に居なくなっている場合 Skip がセットされる
                            // Skip is set if the Actor is already gone.
                ...
                switch (self.selectedBattleCommandType)
                {
                    ...
                    case BattleCommandType.ForceCritical:
                        if (start)
                        {
                            ...
                            if (game.data.system.BattleCameraEnabled[Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_ATTACK])
                            {
                                PlayCameraAnimation(Rom.Camera.NAME_BATTLE_ATTACK);
                            }
                        }
                        else
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                if (game.data.system.BattleCameraEnabled[Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_SKILL])
                                {
                                    ...
                                    camManager.setWaitFunc(() =>
                                    {
                                        ...
                                        var ok = camManager.isSkillCameraPlaying(false) && IsEffectEndPlay && !isWaitInitialize;
                                        return ok;
                                    });
                                }
                    ...
                    case BattleCommandType.Item:
                        if (start)
                        {
                            ...
                            if (actor.frontDir > 0)
                            {
                                ...
                                {
                                    PlayCameraAnimation(Rom.Camera.NAME_BATTLE_USE_ITEM);
                                }
                    ...
                    case BattleCommandType.Change:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                ...
                                {
                                    PlayCameraAnimation(Rom.Camera.NAME_BATTLE_CHANGE);
                                }
                    ...
                    case BattleCommandType.Enter:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                ...
                                {
                                    PlayCameraAnimation(Rom.Camera.NAME_BATTLE_ENTER);
                                }
                    ...
                    case BattleCommandType.Leave:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                ...
                                {
                                    PlayCameraAnimation(Rom.Camera.NAME_BATTLE_LEAVE);
                                }
        ...
        {
            StopCameraAnimation();
            camManager.setCameraFromPathName(catalog, true, Guid.Empty, name);
        ...
        public void draw(SharpKmyGfx.Render scn)
        {
...

            {
                ...
                owner.battleEvents.SetCurrentCameraMatrix(p, v);
        ...
        private void setMapRom(Common.Rom.Map map)
        {
            if (map != null)
            {
                // #13017 マップから借りてきたdrawerの場合、バトル開始時に背景をセットするタイプのフローの時に表示状態のままになることがあるので、一旦無効にしておく
                // #13017 In the case of a drawer borrowed from a map, it may remain displayed during the type of flow that sets the background at the start of a battle, so please disable it once.
                Hide();
                ...
                {
                    battleMapDrawer.setRom(map, false, false, Util.BATTLE3DDISPLAYID, true);
        ...
        private void initializeExtras(Rom.Map map)
        {
            ...
            foreach (var evRef in events)
            {
                ...
                if (ev != null)
                {
                    ...
                    chr.setScale(evRef.scale.X, evRef.scale.Y, evRef.scale.Z);
        ...
        internal void RecoveryConditionMotion(RecoveryStatusInfo recoveryStatusInfo)
        {
            var actor = searchFromActors(recoveryStatusInfo.character);
            if (actor == null)
                return;

            if (actor.lastMotion == recoveryStatusInfo.condition?.motion)
